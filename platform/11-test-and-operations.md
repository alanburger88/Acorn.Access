...
## A.17 Release Certification Checklist

A release train ships only when the release captain can check every box (automated where possible; the checklist itself is generated by CI):

- [ ] All PR-merge gates green on the release SHA (no red-suppressed builds)
- [ ] Full golden-output corpus: 0 unapproved diffs across all formats
- [ ] Accessibility: 0 critical/serious axe violations; 100% PDF/UA validation pass
- [ ] AI eval battery: all thresholds in A.10 met; results archived to evidence store
- [ ] Security: 0 open critical vulns in shipped images; SBOM published; isolation-probe suite green
- [ ] Perf certification: weekly perf run within regression policy (A.12); 72h soak clean
- [ ] Compliance evidence pack generated and validator-complete (A.15)
- [ ] DB migrations rehearsed on staging clone with production-scale data volume
- [ ] Rollback tested: previous version redeployable; new feature flags default-safe (off or ring-0)
- [ ] Runbooks updated for any new operational surface introduced this release
- [ ] On-prem/LTS: upgrade path tested from N-1 and N-2 versions
...
## B.9 Benchmark Methodology & Publication

- **Measurement points are contractual:** each SLI's exact query (PromQL/SQL) is versioned in the SLO catalog repo; latency measured at the platform edge (post-TLS, pre-provider), throughput measured at commit points (document sealed, message accepted by provider).
- Benchmarks run on the published reference footprint (worker shapes in B.1) so customer-VPC deployments can reproduce them; `acornctl bench` ships the exact harness we use.
- Percentiles computed from full-fidelity histograms (no averaged percentiles); 28-day rolling windows for SLO compliance, 5-min windows for alerting.
- A public benchmark sheet (refreshed quarterly) publishes B.1–B.6 achieved-vs-target so sales never quotes stale numbers; internal dashboard tracks trend per release train.
...
**Game-day scenario rotation (each region-pair exercises all six per year):**

| # | Scenario | Success criteria |
|---|---|---|
| GD-1 | Full region evacuation under live synthetic load | RTO ≤ 1h; RPO ≤ 5 min verified by sequence-number audit; 0 duplicate deliveries |
| GD-2 | Control-plane quorum loss (one region partitioned) | Other regions unaffected; partitioned region serves reads; heals without operator data surgery |
| GD-3 | Archive region loss during active retrieval load | Reads fail over < 2 min; sealed-doc RPO = 0 confirmed by hash audit |
| GD-4 | Kafka cluster loss mid-batch | Consumers resume from mirrored offsets; journeys neither skip nor double-fire |
| GD-5 | Mid-failover batch job (10M docs) | Job resumes from checkpoints in DR region; completion penalty ≤ 30 min |
| GD-6 | Regional failback (the often-untested half) | Return to home region with 0 data divergence; reconciliation report clean |
